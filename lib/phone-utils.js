...
(function (global) {
  ...
  // Only match when not immediately preceded or followed by alphanumeric (so "abcd5555555efgh"
  // does not match "5555555" — number must be separated by space, punctuation, or string boundary).
  const PHONE_REGEX = /(?<![a-zA-Z0-9])(?:\+?\d{1,4}[-.\s]?)?\(?\d{2,4}\)?[-.\s]?\d{2,4}[-.\s]?\d{2,4}(?:[-.\s]?\d{1,5})?(?![a-zA-Z0-9])/g;
