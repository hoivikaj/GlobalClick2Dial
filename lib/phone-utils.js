/**
 * Phone number detection and E.164 normalization for GlobalClick2Call extension.
 */
...
(function (global) {
  ...
  // Captures: optional + and country code, then rest of number with common separators.
  const PHONE_REGEX = /(?:\+?\d{1,4}[-.\s]?)?\(?\d{2,4}\)?[-.\s]?\d{2,4}[-.\s]?\d{2,4}(?:[-.\s]?\d{1,5})?/g;
